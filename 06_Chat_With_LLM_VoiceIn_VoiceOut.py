...
def main():
    ...
    if not ELEVENLABS_API_KEY: raise EnvironmentError("ELEVENLABS_API_KEY not found in environment variables.")
    ...
    my_system_instruction += """You are an Expert Site Reliability Engineer with experience in DevOps, DevSecOps, Cloud,
                             ...
                             advanced automation. Give the answer in bulletpoints and keep the answers concise and precise. 
                             If asked to write code, then write code only, do not explain at end."""

    # # User query to send to Gemini AI model
    # user_query = "Tell me a story about me"

    # Initialize Gemini and ElevenLabs client with the API key
    ...
    while True:
        try:
            ...
            # Print the textresponse as it streams in
            print(f"[🤖 LLM]: ", end="")
            for chunk in response:
                print(chunk.text, end="", flush=True)
                full_response_list.append(chunk.text)
                full_response_text = "".join(full_response_list).strip()
            print()  # For newline after the response

            # Convert full_response_text to voice 
            try:
                ...
                print(f"🔴 An Error occured in Text to Speech conversion: {e}")
                recorder.shutdown()
                break
